...
プロジェクトは 3 つの主要モジュールで構成されています:

1. **network**: VPC、サブネット、ルートテーブル、VPC エンドポイント、Route53 プライベートホストゾーンを管理
2. **security**: キーペア、セキュリティグループ、IAM ロール/インスタンスプロフィールを管理
3. **compute_nat**: NAT 機能を持つ Bastion インスタンスを管理
...
- 秘密鍵は Terraform 実行時に自動生成され、ローカルファイルに保存
- bastion.pem: Bastion インスタンス用
- workload.pem: ワークロード用(将来の拡張用)
...
- Security Group: sg-bastion-{project_name}
- EC2 Instance: ec2-bastion-{project_name}
...
- bastion_instance: Bastion インスタンスの詳細設定
- static\_\*: 静的 AMI ID 指定
...
```
terraform/
├── main.tf                    # メインコンフィグ、プロバイダー、モジュール呼び出し
├── variables.tf               # 変数定義
├── outputs.tf                 # 出力定義
...
└── modules/
    ...
    │   ├── variables.tf
    │   └── outputs.tf
    └── compute_nat/           # NAT Bastionモジュール
        ├── main.tf
        ├── variables.tf
        ├── outputs.tf
        └── user-data/
            └── nat-bastion.sh # Bastion初期化スクリプト
```
